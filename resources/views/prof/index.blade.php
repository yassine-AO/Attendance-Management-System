...
<div class="container-fluid d-flex justify-content-center align-items-center background">
    <div class="d-flex flex-column">
        @foreach ($calendars as $calendar)
            @php
                $professorModules = $prof->modules;
                $filteredDays = $calendar->days->filter(function ($day) use ($professorModules) {
                    foreach ($day->modules as $module) {
                        if ($professorModules->contains('id', $module->id)) {
                            return true;
                        }
                    }
                    return false;
            ...
            @if ($filteredDays->isNotEmpty())
                <div class="card m-2 flex-grow-1" style="width: auto;">
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-wrap">
                            @foreach ($filteredDays as $day)
                             @foreach ($day->modules as $module)
                                <a href="{{ route('prof.createRetardsAbsences', ['prof' => $prof->id, 'module' => $module->id]) }}" class="card m-2" style="width: auto; text-decoration: none;">
                                    <div class="card-body">
                                        <h6 class="card-title">{{$day->nom}}</h6>
                                        <ul class="list-group">
                                            
                                                <li class="list-group-item">{{$module->nom}}</li>
