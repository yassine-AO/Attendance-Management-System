...
<form action="{{ route('admin.storeCalendar', $admin->id) }}" method="POST">
    @csrf
    <div class="mb-3">
        ...
        <select class="form-select" id="annee_section" name="annee_section" style="width: 300px;">
            ...
            @php
             $calendarExists = $calendars->where('annee_section_id', $annee_section->id)->isNotEmpty();
            @endphp

              @if ($annee_section->filiere->id == $admin->filiere->id && !$calendarExists)
