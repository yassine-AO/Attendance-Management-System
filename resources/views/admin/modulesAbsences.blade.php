...
<div class="container-fluid d-flex justify-content-center align-items-center background">
    ...
    <table class="table table-dark table-striped">
        <thead>
            ...
            <th scope="col" style="text-decoration: underline;">Absent</th>
            <th scope="col" style="text-decoration: underline;">Motif</th>
        ...
        <tbody>
            @foreach ($absences as $absence)
                @if ($absence->module->annee_section->filiere->campus->id == $admin->filiere->campus->id)
                    <tr>
                        <th scope="row">{{$absence->id}}</th>
                        @if (!$absence->etudiant)
                         <td scope="row">Prof: {{$absence->prof->nom}}</td>
                        @elseif(!$absence->prof)
                         <td scope="row">Etudiant: {{$absence->etudiant->nom}}</td>
                        @endif
                        <td>{{$absence->motif}}</td>
                        <td>{{$absence->module->nom}} | {{$absence->module->annee_section->annee}} | {{$absence->module->annee_section->filiere->nom}}</td>
