...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top" style="background-color: #050050;">
  <div class="container">
    ...
    <a type="button" class="btn btn-primary" href="{{route('admin.createModule', $admin->id)}}">Ajouter Module</a>
...
<div class="container-fluid d-flex justify-content-center align-items-center background">
    ...
    <table class="table table-dark table-striped">
        <thead>
            ...
            <th scope="col" style="text-decoration: underline;">Nom</th>
            <th scope="col" style="text-decoration: underline;">Nombre Des Heures</th>
            <th scope="col" style="text-decoration: underline;">Prof Enseignant</th>
            <th scope="col" style="text-decoration: underline;">Annee de section/Filiere/Campus </th>
            <th scope="col" style="text-decoration: underline;"></th>
            <th scope="col" style="text-decoration: underline;"></th>
            </tr>
        </thead>
        <tbody>
            @foreach ($modules as $module)
                @if ($module->annee_section->filiere->id == $admin->filiere->id)
                    <tr>
                    <th scope="row">{{$module->id}}</th>
                    <td>{{$module->nom}}</td>
                    <td>{{$module->nbrHeures}}</td>
                    <td>{{$module->prof->nom}}</td>
                    <td>{{$module->annee_section->annee}}/{{$module->annee_section->filiere->nom}}/{{$module->annee_section->filiere->campus->nom}}</td>
                    <td> <a href="{{ route('admin.modulesAbsences', ['admin' => $admin->id, 'module' => $module->id]) }}" type="button" class="btn btn-info">Voir les absences</a> </td>
                    <td> <a href="{{ route('admin.modulesNotes', ['admin' => $admin->id, 'module' => $module->id]) }}" type="button" class="btn btn-info">Voir les notes</a> </td>
                    </tr>
                @endif
