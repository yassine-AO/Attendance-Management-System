...
<nav class="navbar navbar-expand-lg navbar-dark" style="background-color: #050050;">
  <div class="container">
    ...
    <a type="button" class="btn btn-primary" href="{{route('admin.createEtudiant', $admin->id)}}">Ajouter Étudiant</a>
...
<div style="background-color: #000656; color: white; padding: 10px; text-align: center;">
  <h3 class="display-3">Les Etudiants:</h3>
</div>
<div class="container-fluid d-flex justify-content-center align-items-center background">
    ...
    <div class="row row-cols-4">
        @foreach ($etudiants as $etudiant)
            @if ($etudiant->annee_section->filiere->id == $admin->filiere->id)

                <div class="col mb-4">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title" style="font-size: 1.2rem;">
                                <u>{{$etudiant->nom}} , {{$etudiant->prenom}}</u>
                            </h5>
                            <p class="card-text">Campus: {{$etudiant->annee_section->filiere->campus->nom}}</p>
                            <p class="card-text">Année de section: {{$etudiant->annee_section->annee}}</p>
                            <p class="card-text">Filière: {{$etudiant->annee_section->filiere->nom}}</p>
                            <a href="{{ route('admin.showEtudiant', ['admin' => $admin->id, 'etudiant' => $etudiant->id]) }}" class="btn btn-dark btn-sm">Parcourir</a>
