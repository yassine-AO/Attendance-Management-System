...
<div class="container-fluid d-flex justify-content-center align-items-center background">
    ...
    <form action="{{ route('admin.updateCalendar',['admin' => $admin->id]) }}" method="POST">
        @csrf
        @method('PUT') 
        <input type="hidden" name="day_id" value="{{ $day->id }}">
        <select class="form-select" size="3" aria-label="Size 3 select example" name="module">
            @foreach ($modules as $module)
                @if ($module->annee_section->filiere->id == $admin->filiere->id && $module->day_id != $day->id && $module->annee_section->id == $calendar->annee_section->id)
                 <option value="{{ $module->id }}">{{ $module->nom }}/{{ $module->annee_section->annee }}/{{ $module->annee_section->filiere->nom }}/{{ $module->annee_section->filiere->campus->nom }}</option>
                @endif
            @endforeach
        </select>
